...
### 4. 无框菜单不抢焦点
`MainWindow` 在 `OnSourceInitialized` 中给 HWND 加 `WS_EX_NOACTIVATE`，确保弹出与点击均不夺取当前应用焦点。菜单全程不抢焦点，原前台窗口始终持焦，故动作执行前只需 `Hide()`，无需显式恢复前台窗口（`GetForegroundWindow` / `SetForegroundWindow` 已在 `NativeMethods` 声明但当前未调用）。
...
拖拽模式（左键按下后）：用 `min/abs` 归一化起止点矩形，支持反向拖拽。松开时按窗口局部坐标 `CroppedBitmap` 裁剪底图 → `Freeze` → `Clipboard.SetImage` → `new PinWindow(crop, screenX, screenY).Show()` 联动钉图（截图罩关闭后贴图存活）。
ESC / 右键取消关闭。
...
- ✅ **8C 阶段已完成（贴图引擎 PinEngine）**：`PinWindow` 贴图常驻，左键拖拽 / 双击关闭 / 右键菜单（置顶·阴影·边界·重置大小·不透明度·旋转·镜像·复制·另存为·作为文件打开·关闭）；`ScreenshotWindow` 结算时 `new PinWindow(crop, screenX, screenY).Show()` 联动钉图，截图罩关闭后贴图窗口存活。
