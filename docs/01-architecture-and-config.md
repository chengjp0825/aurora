...
### `/Models` —— 数据契约
- `ActionItem` —— 实现 `INotifyPropertyChanged` 供 DataGrid 双向绑定；含 `Name` / `Command` / `Arguments` / `Icon`（Segoe MDL2 Assets hex 码，菜单按钮经 `HexToGlyphConverter` 转字形显示）。
- `SettingsModel` —— 多层级 POCO，默认值对齐重构前硬编码：
  ...
  - `SnippingSettings`（`BorderColor` 红框色 / `MaskAlpha` 暗罩浓度 / 阈值 / `AfterScreenshot` 截图后行为 / `CaptureScope` 截图范围）/ `MenuSettings`（颜色 / 尺寸 / 圆角）。
...
关键视觉与交互参数（`Snipping` / `Menu` / `Pin` 的颜色、浓度、尺寸、阈值、`AfterScreenshot` 截图后行为、`CaptureScope` 截图范围、`InterceptWakeupKey` 吞键开关、`CircleSensitivity` 画圈灵敏度、`DefaultTopmost` / `DefaultShowShadow` 等）。各窗口构造函数 `InitializeComponent()` 后读 `SettingsManager.Instance.Settings.{组}` 动态赋值给命名控件属性；按钮背景色等 Style 内部值经 `{DynamicResource}` 注入（窗口 `Resources[key] = BrushHelper.ToBrush(...)`）。
