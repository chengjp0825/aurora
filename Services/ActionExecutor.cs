...
/// <summary>
/// Loads actions from appsettings.json (via SettingsManager) and executes
...
internal sealed class ActionExecutor
{
    ...
    /// <summary>
    /// Returns the current action list, freshly loaded from appsettings.json
    /// so edits made in the settings window are reflected on the next wake-up.
    /// </summary>
    public List<ActionItem> GetActions() => _settingsManager.Load().Actions;
