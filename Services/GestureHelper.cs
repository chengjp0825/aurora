...
/// 调用方（GlobalHookService）负责维护滑动时间窗并剔除老点，并按灵敏度预设注入
/// <paramref name="minBoxSide"/> / <paramref name="minTotalTurnDeg"/>；本方法只做空间几何
/// 判定，O(n) 单趟扫描，n≈50~120 点时开销可忽略。
/// </summary>
internal static class GestureHelper
{
    ...
    /// <paramref name="minBoxSide"/> 外接矩形最小边（排除微小抖动），
    /// <paramref name="minTotalTurnDeg"/> 累计有符号偏转角阈值（接近一圈），均由调用方
    /// 按 <see cref="MyQuicker.Models.CircleSensitivity"/> 预设注入。
    /// </summary>
    public static bool IsCircle(List<POINT> recentPoints, double minBoxSide, double minTotalTurnDeg)
    {
        if (recentPoints is null || recentPoints.Count < MinPoints)
            return false;

        // 1) Bounding Box：宽高均 ≥ minBoxSide，宽高比 0.5~2.0
        ...
        //    取总数的绝对值 ≥ minTotalTurnDeg 即判定为近闭合圆。
